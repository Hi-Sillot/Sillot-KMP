 ...
 * Copyright (c) 2024.
 *
 * lastModified: 2024/9/2 上午10:31
 * updated: 2024/9/2 上午10:31
 */

@file:Suppress("CompositionLocalNaming", "CompositionLocalNaming")

package sc.hwd.sillot.kmp.producer
...
import sc.hwd.loftus.ld246.HomeActivity
import sc.hwd.sillot.gibbet.producer.GibbetPro
import sc.hwd.sillot.gibbet.services.GibbetKernelServiceManager
import sc.hwd.sofill.S
import sc.hwd.sofill.Ss.S_Compose
import sc.hwd.sofill.Ss.S_Uri
import sc.hwd.sofill.U
import sc.hwd.sofill.Us.Toast
import sc.hwd.sofill.Us.U_DOSC
import sc.hwd.sofill.Us.U_FileUtils
import sc.hwd.sofill.Us.U_FileUtils.workspaceParentDir
import sc.hwd.sofill.Us.U_Uri.openUrl
import sc.hwd.sofill.Us.displayTokenLimiter
import sc.hwd.sofill.Us.encryptAes
import sc.hwd.sofill.Us.generateAesKey
import sc.hwd.sofill.Us.getDecryptedToken
...
import sc.hwd.sofill.compose.ApkButtons
import sc.hwd.sofill.compose.AudioButtons
import sc.hwd.sofill.compose.LockScreenOrientation
import sc.hwd.sofill.compose.MagnetButtons
import sc.hwd.sofill.compose.MultiIndexNavigationBottomBar
import sc.hwd.sofill.compose.SelectableHtmlText
import sc.hwd.sofill.compose.SelectableText
import sc.hwd.sofill.compose.VideoButtons
import sc.hwd.sofill.compose.components.CommonTopAppBar
import sc.hwd.sofill.compose.components.WaitUI
import sc.hwd.sofill.compose.partialCom.DdMenuI
import sc.hwd.sofill.compose.partialCom.NetworkAware
import sc.hwd.sofill.compose.theme.CascadeMaterialTheme
import sc.hwd.sofill.pioneer.mmkv
...
class MainPro : ComponentActivity(), IGibbetKernelServiceAction {
...
//        setIntent(intent) // 更新当前 Intent
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        GibbetKernelServiceManager(this).bindService(this, WebPoolsPro.key_MainPro, this)
    }
    ...
    fun MainProUI() {
        ...
        LaunchedEffect(key1 = in2_intent) {
            ...
            Gibbet_kernel_API_token.value = getDecryptedToken(
                mmkv.value, S.KEY_TOKEN_Sillot_Gibbet_kernel_API, S.KEY_AES_TOKEN_Sillot_Gibbet_kernel_API
            )
            siyuan_kernel_API_token.value = getDecryptedToken(
                mmkv.value, S.KEY_TOKEN_siyuan_kernel_API, S.KEY_AES_TOKEN_siyuan_kernel_API
            )
            when (in2_intent?.action) {
                Intent.ACTION_SEND -> {
                    ...
                    in2_intent?.getParcelableExtra<Uri>(Intent.EXTRA_STREAM)?.let { sharedFileUri ->
                        ...
                        fileName.value =
                            U_FileUtils.getFileName(thisActivity, sharedFileUri).toString()
                        fileSize.value = U_FileUtils.getFileSize(thisActivity, sharedFileUri)
                        mimeType.value =
                            U_FileUtils.getMimeType(thisActivity, sharedFileUri).toString()
                        fileType.value = U_FileUtils.getFileMIMEType(mimeType.value, fileName.value)
                            ?: U_FileUtils.getFileMIMEType(mimeType.value)
                    }
                }
                ...
                Intent.ACTION_SEND_MULTIPLE -> {
                    ...
                    in2_intent?.data?.let {
                        fileUri.value = it
                        fileName.value = U_FileUtils.getFileName(
                            thisActivity, it
                        ).toString()
                        fileSize.value = U_FileUtils.getFileSize(
                            thisActivity, it
                        )
                        mimeType.value = U_FileUtils.getMimeType(
                            thisActivity, it
                        ).toString()
                        fileType.value = fileName.value.let { it1 ->
                            U_FileUtils.getFileMIMEType(mimeType.value, it1)
                        }
                    } ?: {
                        mimeType.value = ""
                        fileType.value = U_FileUtils.getFileMIMEType(mimeType.value)
                    }
                }
            }
        ...
        LaunchedEffect(currentIndex.intValue) {
            ...
            fileUris.value[currentIndex.intValue - 1]?.let {
                fileUri.value = it
                fileName.value = U_FileUtils.getFileName(thisActivity, it).toString()
                fileSize.value = U_FileUtils.getFileSize(thisActivity, it)
                mimeType.value = U_FileUtils.getMimeType(thisActivity, it).toString()
                fileType.value = U_FileUtils.getFileMIMEType(mimeType.value, fileName.value)
                    ?: U_FileUtils.getFileMIMEType(mimeType.value)
        ...
        Scaffold(
            ...
            Box(
                ...
                if (in2_intent?.action == Intent.ACTION_SEND && in2_intent?.type?.startsWith("text/") == true) {
                    ...
                    val html: String
                    // 判断文本类型并进行合法性校验
                    when (U_DOSC.checkContentFormat(sharedText)) {
                        "HTML" -> {
                            // 处理HTML文本
                            html = U_DOSC.processHtml(sharedText)
                        }

                        "Markdown" -> {
                            // 处理Markdown文本
                            html = U_DOSC.processMarkdown(sharedText)
                        }

                        else -> {
                            // 处理普通文本
                            html = U_DOSC.processPlainText(sharedText)
                        }
                    }
                    LazyColumn(
                        modifier = Modifier
...
//                            .height(IntrinsicSize.Min)
            ...
            cb = {
                ...
                InputDialog("🛸 API TOKEN",
                    ...
                    .setOkButton { baseDialog, v, inputStr ->
                        ...
                        // 加密Token
                        val encryptedToken = encryptAes(inputStr, aesKey)
                        // 将加密后的Token存储到MMKV中
                        mmkv.value.encode(S.KEY_AES_TOKEN_Sillot_Gibbet_kernel_API, encodedKey)
                        mmkv.value.encode(S.KEY_TOKEN_Sillot_Gibbet_kernel_API, encryptedToken)
            ...
            cb = {
                ...
                InputDialog("🛸 API TOKEN",
                    ...
                    .setOkButton { baseDialog, v, inputStr ->
                        ...
                        val encryptedToken = encryptAes(inputStr, aesKey)
                        // 将加密后的Token存储到MMKV中
                        mmkv.value.encode(S.KEY_AES_TOKEN_siyuan_kernel_API, encodedKey)
                        mmkv.value.encode(S.KEY_TOKEN_siyuan_kernel_API, encryptedToken)
                        PopNotification.show(
                            "TOKEN已更新（${
                                displayTokenLimiter(
                                    inputStr, "token ".length + 4, 4
    ...
    fun SendBtnPart(
        ...
        Button(modifier = Modifier
            .width(S_Compose.Button_Width.current.dp)
            .padding(top = if (isLandscape) S_Compose.btn_PaddingTopH.current else S_Compose.btn_PaddingTopV.current),
            colors = ButtonDefaults.buttonColors(
                containerColor = S_Compose.btn_bgColor3.current, contentColor = S_Compose.btn_Color1.current
            ),
            enabled = true,
            onClick = {
                ...
                if (markdown != null) {
                    val directories =
                        U_FileUtils.getDirectoriesInPath(thisActivity.workspaceParentDir())
            ...
            Text(
                text = S_Compose.btnText5.current,
                letterSpacing = S_Compose.btn_lspace.current,
                fontSize = if (isLandscape) S_Compose.btn_TextFontsizeH.current else S_Compose.btn_TextFontsizeV.current
            )
        }
        Button(modifier = Modifier
            .width(S_Compose.Button_Width.current.dp)
            ...
            enabled = true,
            onClick = {
                if (GibbetKernelServiceManager(thisActivity).service == null) {
                    PopNotification.show(
            ...
            Text(
                text = S_Compose.btnText6.current,
                letterSpacing = S_Compose.btn_lspace.current,
                fontSize = if (isLandscape) S_Compose.btn_TextFontsizeH.current else S_Compose.btn_TextFontsizeV.current
            )
        }
    ...
    fun InfoPart(
        ...
        val inspectionMode = LocalInspectionMode.current // 获取当前是否处于预览模式// 获取窗口尺寸
        val Thumbnail_Height = S_Compose.Thumbnail_Height.current
        val Thumbnail_Height_IMG = S_Compose.Thumbnail_Height_IMG.current
        ...
        Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            Column(
                ...
                if (fileType.value.endsWith("图像")) {
                    ...
                    val icon = U_FileUtils.getIconForFileType(fileType.value)
                    Icon(
                        imageVector = icon,
            ...
            coroutineScope.launch {
                withContext(Dispatchers.IO) {
                    try {
                        ...
                        val sourceFilePath = U_FileUtils.getPathFromUri(thisActivity, uri_from_file)
                        // 复制文件到所选文件夹
                        fileName.value.let {
                            sourceFilePath?.let { it1 ->
                                U_FileUtils.copyFileToFolderByDocumentTree(
                ...
                withContext(Dispatchers.IO) {
                    try {
                        ...
                        fileName.value.let {
                            sourceFilePath?.let { it1 ->
                                try {
                                    U_FileUtils.copyFileToMyAppFolder(
                                        workspaceAssetsDir, it, it1
                                    )
                                    GibbetKernelServiceManager(thisActivity).service?.apply {
                                        goUpdateAssets()
                                        goReindexAssetContentOnce()
                                        goIncSyncOnce()
                                    }
//                                    Mobile.updateAssets()
...
//                                    Mobile.incSyncOnce()
            ...
            rememberLauncherForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
                ...
                if (result.resultCode == RESULT_OK) {
                    result.data?.data?.let { _uri ->
                        ...
                        sharedFileUri?.let {
                            uri_from_file = U_FileUtils.getFileFromUri(
                                thisActivity, it
                            )?.toUri()
                            uri_to_dir = _uri
        ...
        if (showSaveButton) {
            ...
            Button(modifier = Modifier
                ...
                Text(
                    text = S_Compose.btnText3.current,
                    letterSpacing = S_Compose.btn_lspace.current,
                    fontSize = if (isLandscape) S_Compose.btn_TextFontsizeH.current else S_Compose.btn_TextFontsizeV.current
                )
            }
            ...
            Button(modifier = Modifier
                ...
                .padding(top = if (isLandscape) S_Compose.btn_PaddingTopH.current else S_Compose.btn_PaddingTopV.current),
                colors = ButtonDefaults.buttonColors(
                    containerColor = S_Compose.btn_bgColor4.current, contentColor = S_Compose.btn_Color1.current
                ),
                enabled = true,
                onClick = {
                ...
                Text(
                    text = S_Compose.btnText4.current,
                    letterSpacing = S_Compose.btn_lspace.current,
                    fontSize = if (isLandscape) S_Compose.btn_TextFontsizeH.current else S_Compose.btn_TextFontsizeV.current
                )
            }
        ...
        fun MagnetBTNonClick1() {
            in2_intent?.data?.let {
                thisActivity.openUrl(it.toString(), true)
            }
        }
        if (inspectionMode || showAudioButton) {
            ...
            ApkButtons(S_Compose.btnText5Apk1.current, ::ApkBTNonClick1)
            ApkButtons(S_Compose.btnText5Apk2.current, ::ApkBTNonClick2)
        } else if (showMagnetButton) {
            MagnetButtons(S_Compose.btnTextOpenByThirdParty.current, ::MagnetBTNonClick1)
        }
    ...
    fun DefaultPreview() {
        ...
        CompositionLocalProvider(
            S_Compose.Thumbnail_Height provides 250,
            S_Compose.Button_Width providesDefault 300,
        ) {
            MainProUI()
        }
    }
