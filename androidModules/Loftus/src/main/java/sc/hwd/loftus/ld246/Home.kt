 ...
 * Copyright (c) 2024.
 *
 * lastModified: 2024/8/25 下午12:55
 * updated: 2024/8/28 上午12:06
 */

package sc.hwd.loftus.ld246
...
import sc.hwd.sofill.S.Matrix_ld246
import sc.hwd.sofill.Ss.S_API
...
import sc.hwd.sofill.android.HWs
...
import sc.hwd.sofill.api.MyRetrofit.createRetrofit
import sc.hwd.sofill.api.ld246.ApiServiceNotification
import sc.hwd.sofill.compose.FullScreenWebView
import sc.hwd.sofill.compose.MyTagHandler
...
import sc.hwd.sofill.interfaces.loftus.ld246_Response
import sc.hwd.sofill.interfaces.loftus.ld246_Response_Data_Notification
import sc.hwd.sofill.interfaces.loftus.ld246_User
import sc.hwd.sofill.pioneer.getSavedValue
import sc.hwd.sofill.pioneer.mmkv
import sc.hwd.sofill.pioneer.rememberSaveableMMKV
import sc.hwd.sofill.pioneer.rememberSerializableMMKV
import sc.hwd.sofill.pioneer.savableStateFlowMMKV
import sc.hwd.sofill.pioneer.saveValue
...
class HomeActivity : MatrixModel() {
    ...
    private var openUrlExternal: Boolean =
        mmkv.value.getSavedValue("${S_Intent.汐洛}_@openUrlExternal", false) // 全局同步配置
    ...
    private val mapEmpty = mutableMapOf<String, List<ld246_Response_Data_Notification>?>().apply {
        S_API.ld246_notification_type.associateWithTo(this) { emptyList() }
    }
...
//                        exitProcess(0)
    ...
    private fun UI(intent: Intent?) {
        ...
        val currentTab = rememberSaveableMMKV(mmkv.value, "${srcPath}_@currentTab", "用户")
        val isShowBottomText = rememberSaveableMMKV(mmkv.value, "${srcPath}_@isShowBottomText", false)
        val pullToRefreshState = rememberPullToRefreshState()
        val isMenuVisible = rememberSaveable { mutableStateOf(false) }
        val userPageData = rememberSerializableMMKV(mmkv.value, "${srcPath}_@userPageData", ld246_User())
    ...
    private fun AddDropdownMenu(
        ...
        DdMenuI(
            ...
            cb = {
                ...
                mmkv.value.saveValue("${S_Intent.汐洛}_@openUrlExternal", openUrlExternal)
            }
        )
        DdMenuI(
            ...
            cb = {
                onDismiss()
                val deToken = getDecryptedToken(mmkv.value, S.KEY_TOKEN_ld246, S.KEY_AES_TOKEN_ld246)
                InputDialog(
                    "🛸 API TOKEN",
                    ...
                    .setOkButton { baseDialog, v, inputStr ->
                        ...
                        // 将加密后的Token存储到MMKV中
                        mmkv.value.encode(S.KEY_AES_TOKEN_ld246, encodedKey)
                        mmkv.value.encode(S.KEY_TOKEN_ld246, encryptedToken)
                        pullToRefreshState.startRefresh()
    ...
    private fun SecondaryTextTabs(
        ...
        val state by rememberSaveable { mutableIntStateOf(S_API.ld246_notification_type.size) }
        val selectedContentColor = S_Compose.btn_bgColor_pink.current    // 选中时文字颜色
        ...
        Column(modifier = modifier) {
            TabRow(
                ...
                S_API.ld246_notification_type.forEachIndexed { index, title ->
                    Tab(
...
//                    delay(100) // 避免接口请求频繁
    ...
    /**
     * 完全体（CustomLinkMovementMethod），因为需要共享一些数据不好抽离，基础版在 [sc.hwd.sofill.compose.SelectableHtmlText]
     */
    @Composable
