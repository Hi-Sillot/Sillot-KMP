 ...
 * Copyright (c) 2024.
 *
 * lastModified: 2024/9/2 上午8:42
 * updated: 2024/9/2 上午8:42
 */

package sc.hwd.sillot.gibbet.producer
...
import sc.hwd.sofill.Us.U_FuckOtherApp.launchSiyuan
import sc.hwd.sofill.Us.formatFull1
import sc.hwd.sofill.api.MyRetrofit.createRetrofit
import sc.hwd.sofill.api.siyuan.SiyuanNoteAPI
import sc.hwd.sofill.api.siyuan.siyuan
import sc.hwd.sofill.interfaces.gibbet.ICreateDocWithMdRequest
import sc.hwd.sofill.interfaces.gibbet.INotebook
                            ...
                            siyuan.Works.createNote(api, payload, token) { success, info, createNote_response ->
                                if (success) {
                                    ...
                                    createNote_response?.body()?.data?.let {
                                        thisActivity.launchSiyuan("siyuan://blocks/$it")
                                    }
                                } else {
...
//        val response = Mobile.getNotebooks(false)
        GibbetKernelServiceManager(thisActivity).service?.goGetNotebooks(false)?.value.let { response ->
            if (response == null || response.error?.isNotEmpty() == true) {
                BuglyLog.e(TAG, "Error: ${response?.error}")
            } else {
                val notebooksJSON = response.notebooksJSON
                val notebooks =
                    Json.decodeFromString<List<INotebook>>(notebooksJSON)
    ...
    //                            TODO 有空解决
//                                GibbetKernelServiceManager(thisActivity).goCreateDocWithMd(paramsJSON)?.value?.let { response2 ->
//                                    // 切换回主线程更新UI
//                                    thisActivity.runOnUiThread {
//                                        if (response2.error.isEmpty()) {
//                                            BuglyLog.i(TAG, "Created document with ID: ${response2.id}")
//                                            thisActivity.startGibbetActivityWithBlock(
//                                                MainActivity::class.java,
//                                                "siyuan://blocks/${response2.id}",
//                                                !MainActivity.isInstanceCreated
//                                            )
//                                        } else {
