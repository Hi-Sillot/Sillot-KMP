 ...
 * Copyright (c) 2024.
 *
 * lastModified: 2024/9/2 上午8:41
 * updated: 2024/9/2 上午8:41
 */
...
import sc.hwd.sofill.Ss.QQ
import sc.hwd.sofill.Ss.QQMail
import sc.hwd.sofill.Ss.REQUEST_CAMERA
import sc.hwd.sofill.Ss.抖音
import sc.hwd.sofill.Us.U_DEBUG
import sc.hwd.sofill.Us.U_DOSC
import sc.hwd.sofill.Us.U_FuckOtherApp.launchQQAndCopyToClipboard
import sc.hwd.sofill.Us.U_FuckOtherApp.launchTikTopAndCopyToClipboard
import sc.hwd.sofill.Us.U_FuckOtherApp.sendEmail
import sc.hwd.sofill.Us.U_Phone
import sc.hwd.sofill.Us.U_Pro.ContentInsertCallback
import sc.hwd.sofill.Us.U_Pro.onDragSend2Producer
import sc.hwd.sofill.Us.U_Pro.onDragWebView
...
import sc.hwd.sofill.interfaces.gibbet.IInsertBlockNextRequest
import sc.hwd.sofill.pioneer.mmkvGibbet
import java.io.File
 ...
 * @see [onDragWebView]
 */
fun WebView.onDragInsertIntoWebView(event: DragEvent, context: Context) {
    ...
    this.onDragWebView(event, object : ContentInsertCallback {
        override fun onStringContentInsert(content: String, webView: WebView) {
            ...
            webView.evaluateJavascript(
                ...
                val md = U_DOSC.text2Markdown(content)
                value?.let {
...
//                    Mobile.insertBlockNext(paramsJSON)
                    GibbetKernelServiceManager(context.applicationContext).service?.goInsertBlockNext(paramsJSON)
                }
            }
        }
        ...
        override fun onFileContentInsert(file: File, webView: WebView) {
            // 如果是文件，转发处理
            webView.onDragSend2Producer(event, "sc.hwd.sillot.kmp.producer.MainPro")
        }
...
fun Activity.androidFeedback() {
    ...
    BottomMenu.show(menuOptions)
        ...
        .setOnMenuItemClickListener { dialog, text, index ->
            when (text) {
                "电子邮件" -> {
                    sendEmail(packageManager, QQMail, "汐洛安卓反馈", U_DEBUG.getDeviceInfoString())
                }
                "QQ" -> {
                    launchQQAndCopyToClipboard(QQ, "开发者 QQ 号已复制")
                }
                "抖音" -> {
                    launchTikTopAndCopyToClipboard(抖音, "开发者抖音号已复制")
                }
            }
            false
        }
...
fun setSillotGibbetCheckInState(webView: WebView): Boolean {
    ...
    runOnUiThread {
        ...
        mmkvGibbet.value.putString("AppCheckInState", if (result) "lockScreen" else "unlockScreen")
...
fun Activity.coldRestart(cl: Class<out Activity>, webView: WebView) {
    ...
    Process.killProcess(Process.myPid()) // 暂时无法解决杀死其他任务栈的冲突，不加这句无法重启内核
}
