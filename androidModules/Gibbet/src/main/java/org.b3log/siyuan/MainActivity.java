...
package org.b3log.siyuan;
 ...
 import static sc.hwd.sillot.gibbet.GibbetMainActivityHelperKt.androidFeedback;
 import static sc.hwd.sillot.gibbet.GibbetMainActivityHelperKt.coldRestart;
 import static sc.hwd.sillot.gibbet.GibbetMainActivityHelperKt.handleKeyEvent;
 import static sc.hwd.sillot.gibbet.GibbetMainActivityHelperKt.handleOnBack;
 import static sc.hwd.sillot.gibbet.GibbetMainActivityHelperKt.onDragInsertIntoWebView;
 import static sc.hwd.sillot.gibbet.GibbetMainActivityHelperKt.openCamera;
 import static sc.hwd.sillot.gibbet.GibbetMainActivityHelperKt.setSillotGibbetCheckInState;
 import static sc.hwd.sofill.Ss.S_REQUEST_CODEKt.REQUEST_CAMERA;
 import static sc.hwd.sofill.Ss.S_REQUEST_CODEKt.REQUEST_IGNORE_BATTERY_OPTIMIZATIONS_AND_REBOOT;
 import static sc.hwd.sofill.Ss.S_REQUEST_CODEKt.REQUEST_OVERLAY;
 import static sc.hwd.sofill.Ss.S_REQUEST_CODEKt.REQUEST_SELECT_FILE;
 import static sc.hwd.sofill.Us.U_FileUtils.getMimeTypeForHTTP;
 import static sc.hwd.sofill.Us.U_LayoutKt.applyStatusBarConfigurationV2;
 import static sc.hwd.sofill.Us.U_WebviewKt.checkWebViewVer;
 import static sc.hwd.sofill.Us.U_WebviewKt.showJSAlert;
 import static sc.hwd.sofill.android.services.FloatingWindowServiceKt.getFloatingWindowServiceManager;
 import static sc.hwd.sofill.pioneer.StoreKt.mmkvGibbet;
 ...
 import sc.hwd.sofill.Us.U_WebviewKt;
 import sc.hwd.sofill.android.MatrixModel;
 import sc.hwd.sofill.Ss.S_Webview;
 import sc.hwd.sofill.Us.U_DEBUG;
 import sc.hwd.sofill.Us.U_Permission;
 ...
 import sc.hwd.sofill.android.webview.WebViewLayoutManager;
...
public class MainActivity extends MatrixModel implements IGibbetKernelServiceAction {
    private final String TAG = "MainActivity-SiYuan";
     private final String Matrix_model = S.Matrix_Gibbet;
     @NonNull
     @Override
    ...
    private GibbetKernelServiceManager gibbetKernelServiceManager;
//    private BootServiceManager bootServiceManager;
    ...
    public boolean dispatchKeyEvent(@NonNull KeyEvent event) {
     ...
     public void onPointerCaptureChanged(boolean hasCapture) {
         ...
         BuglyLog.d(TAG, "onPointerCaptureChanged: " + hasCapture);
     }
    ...
    public void onNewIntent(final Intent intent) {
     ...
     private void init(final Intent intent) {
        ...
        if (!isNullOrEmpty(blockURL) && null != webView) {
      ...
      */
     @Override
     public void performActionWithService() {
    ...
    protected void onCreate(final Bundle savedInstanceState) { // 只执行一次。在这里设置布局和初始化数据。在大多数情况下，不需要在 onRestart 中做太多事情，因为 onStart 已经处理了活动可见时的初始化。
        ...
        gibbetKernelServiceManager = new GibbetKernelServiceManager(getApplicationContext());
        gibbetKernelServiceManager.bindService(this, WebPoolsPro.key_SG, this);
        gibbetKernelServiceManager.onServiceReady((it) -> {
            init(getIntent());
            return null;
        });
    ...
    private void initUIElements() {
        ...
        if (webView != null) {
            ...
            webView.setOnDragListener((view, event) -> {
                ...
                onDragInsertIntoWebView(webView, event, view.getContext());
        ...
        webView.setWebViewClient(new WebViewClient() {
            ...
            public boolean shouldOverrideUrlLoading(final WebView view, final WebResourceRequest request) {
                ...
                if (url.contains("127.0.0.1:58131")) {
                    String AppCheckInState = mmkvGibbet.getValue().getString("AppCheckInState", "");
...
//                    coldRestart();
             ...
             *  {@link WebChromeClient} 的 onProgressChanged 方法监听也是一样的
             */
            @Override
        ...
        U_WebviewKt.applyDefault(ws, 100,
                "SiYuan-Sillot/" + SillotApplication.getInstance().getVersionName() + " https://b3log.org/siyuan Android " + ws.getUserAgentString());

        // 使用loadDataWithBaseURL方法加载HTML内容 并不能解决小米手机禁用APP的网络无法加载的问题，因为 HTML 脚本里依旧有 http 请求。
        webView.loadUrl("127.0.0.1:58131/appearance/boot/index.html?v=" + SillotApplication.getInstance().getVersionName());
...
//    }
    ...
    public void onForeground(@NonNull Activity activity) {
        ...
        Objects.requireNonNull(getFloatingWindowServiceManager().getValue())
                .startFloatingWindowService(this, false);
    }
    ...
    public void onBackground(@NonNull Activity activity) {
        ...
        Objects.requireNonNull(getFloatingWindowServiceManager().getValue())
                .startFloatingWindowService(this, true);
    }
