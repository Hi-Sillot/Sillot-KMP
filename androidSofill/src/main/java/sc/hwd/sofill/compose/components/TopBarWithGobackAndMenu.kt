 ...
 * Copyright (c) 2024.
 *
 * lastModified: 2024/8/25 下午12:59
 * updated: 2024/8/25 下午12:59
 */
...
import sc.hwd.sofill.Us.U_Phone.toggleFullScreen
import sc.hwd.sofill.Us.U_Uri.openUrl
import sc.hwd.sofill.Us.disableScreenshot
import sc.hwd.sofill.Us.enableScreenshot
...
fun TopRightMenu(
    ...
    val isFullScreen = rememberSaveableMMKV(mmkv.value, "${srcPath}_@isFullScreen", false) { _, it ->
        activity.toggleFullScreen(it.value)
    }
    var canCaptureScreenshot =
        rememberSaveableMMKV(mmkv.value, "${srcPath}_@canCaptureScreenshot", true) { _, it ->
            if (it.value) {
            ...
            leadingIcon = {
                Icon(
                    ...
                    modifier = modifier.size(
                        S_Compose.small_iconSize.current
                    )
                )
            },
            children = {
                ...
                DropdownMenuItem(
                    ...
                    onClick = { onDismiss();activity.openUrl("${S_Uri.gitRepoUrl}/issues/new") },
                )
                DropdownMenuItem(
                    text = { Text("查看源码") },
                    leadingIcon = { Icon(Icons.TwoTone.Code, contentDescription = null) },
                    onClick = { onDismiss();activity.openUrl("${S_Uri.gitRepoUrl}/blob/HEAD/app/src/main/java/${srcPath}") },
                )
            },
        )
    }
