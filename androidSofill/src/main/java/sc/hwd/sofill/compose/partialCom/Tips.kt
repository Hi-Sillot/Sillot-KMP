...
package sc.hwd.sofill.compose.partialCom
...
import sc.hwd.sofill.compose.NetworkViewModelFlow
...
fun NetworkAware() {
    Box(
        ...
        NetworkAwareContent(NetworkViewModelFlow(SillotApplication.getInstance()))
    }
}
...
fun NetworkAwareContent(viewModel: NetworkViewModelFlow) {
    ...
    if (!isNetworkAvailable) {
        ...
        Row(
            modifier = Modifier
                .background(S_Compose.Card_bgColor_red1.current)
                .fillMaxWidth()
                .clickable {
