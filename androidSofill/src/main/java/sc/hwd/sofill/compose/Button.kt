 ...
 * Copyright (c) 2024.
 *
 * lastModified: 2024/8/25 上午1:05
 * updated: 2024/8/25 上午1:05
 */
...
fun AudioButtons(uri: Uri) {
    ...
    val Button_Width = S_Compose.Button_Width.current
    val btn_lspace = S_Compose.btn_lspace.current
    val btn_PaddingTopH = S_Compose.btn_PaddingTopH.current
    val btn_PaddingTopV = S_Compose.btn_PaddingTopV.current
    val btn_TextFontsizeH = S_Compose.btn_TextFontsizeH.current
    val btn_TextFontsizeV = S_Compose.btn_TextFontsizeV.current
    ...
    Button(modifier= Modifier
        ...
        colors = ButtonDefaults.buttonColors(
            containerColor = S_Compose.btn_bgColor_pink.current,
            contentColor = S_Compose.btn_Color1.current
        ), enabled = false,
        onClick = { /*TODO*/ }) {
        Text(
            text = S_Compose.btnTextAudio1.current,
    ...
    Button(modifier= Modifier
        ...
        Text(
            text = S_Compose.btnTextOpenByThirdParty.current,
            letterSpacing = btn_lspace,
            fontSize = if (isLandscape) btn_TextFontsizeH else btn_TextFontsizeV
        )
    }
...
fun VideoButtons(uri: Uri) {
    ...
    Button(modifier= Modifier
        ...
        Text(
            text = S_Compose.btnTextVideo1.current,
            letterSpacing = btn_lspace,
            fontSize = if (isLandscape) btn_TextFontsizeH else btn_TextFontsizeV
        )
    }
...
fun ApkButtons(btnText: String,  cb: () -> Unit) {
    ...
    val btn_TextFontsizeH = S_Compose.btn_TextFontsizeH.current
    val btn_TextFontsizeV = S_Compose.btn_TextFontsizeV.current
    val btn_Color1 = S_Compose.btn_Color1.current
    val btn_bgColor1 = S_Compose.btn_bgColor_pink.current
    val isLandscape = LocalConfiguration.current.orientation == Configuration.ORIENTATION_LANDSCAPE // 是否横屏（宽高比）
    Button(modifier= Modifier
        .width(Button_Width.dp)
        .padding(top = if (isLandscape) btn_PaddingTopH else btn_PaddingTopV),
