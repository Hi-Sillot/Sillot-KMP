 ...
 * Copyright (c) 2024.
 *
 * lastModified: 2024/8/22 下午10:22
 * updated: 2024/8/22 下午10:22
 */
...
import sc.hwd.sofill.compose.WebViewActivity
...
object U_Uri {
    ...
    fun Activity.openUrl(url: String, noBrowser: Boolean = false) {
     ...
     */
    @JvmStatic
    fun Activity.openURLUseDefaultApp(url: String) {
    ...
    @JvmStatic
    fun Activity.openURLUseSB(url: String) {
        val uri = Uri.parse(url)
    ...
    @JvmStatic
    fun Activity.openURLUseSBInSplitMode(url: String) {
        val uri = Uri.parse(url)
        Intent(this, WebViewActivity::class.java).apply {
            setData(uri)
            addFlagsForMultiSplit()
        }.also {
            startActivity(it)
        }
    }
 ...
 * 适用于 Activity 嵌入
 */
fun Intent.addFlagsForSplitActivity(): Intent {
    this.apply {
        addFlags(
...
/**
 * 分屏显示新窗口, 但是在小米系统上会存在一段时间的窗口调整期, 期间窗口不可交互， 而且不便于重复调用
 */
