...
object ClipboardUtils {
    ...
    fun copyText(text: CharSequence?) {
        val cm =  SillotApplication.getInstance().getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
        cm.setPrimaryClip(ClipData.newPlainText( SillotApplication.getInstance().getPackageName(), text))
    }
    ...
    fun getLabel(): CharSequence {
        val cm =  SillotApplication.getInstance()
    ...
    val text: CharSequence
        get() {
            val cm =
                SillotApplication.getInstance().getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
            val clip = cm.primaryClip
            if (clip != null && clip.itemCount > 0) {
                val text = clip.getItemAt(0).coerceToText( SillotApplication.getInstance())
                if (text != null) {
                    return text
                }
            }
