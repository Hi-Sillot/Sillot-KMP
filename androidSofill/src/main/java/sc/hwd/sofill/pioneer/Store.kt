 ...
 * Copyright (c) 2024.
 *
 * lastModified: 2024/8/29 上午12:51
 * updated: 2024/8/29 上午12:51
 */

package sc.hwd.sofill.pioneer
...
fun Context.getSavedBoolean(key: String, defaultValue: Boolean): Boolean {
    val sharedPreferences = getSharedPreferences(S_Intent.汐洛, Context.MODE_PRIVATE)
...
inline fun <reified T : Any> rememberSerializableMMKV(
    ...
    val json = Json { ignoreUnknownKeys = true } // 创建Json实例，可以根据需要配置
 ...
 */
@JvmField
val mmkv = lazy { MMKV.defaultMMKV() }

/**
 * 使用前确保 MMKV 已初始化
 */
@JvmField
val mmkvGibbet = lazy { MMKV.mmkvWithID("Gibbet", MMKV.MULTI_PROCESS_MODE) }
